...
public class TestPlanetGenerator extends PlanetGenerator {
	...
	protected void generate() {
		...
		// create rooms
		for (int i = 0; i < 15; i++) {
			Vec2 rotate = Tmp.v1.trns(noise3d(((int) sector.tile.v.y/sector.tile.v.x) * 10 + i, sector.tile.v, 3, 0.5f, 200f, 720f), width/(2.5f + noise3d(((int) sector.tile.v.y/sector.tile.v.z) * i + 10, sector.tile.v, 3, 0.5f, 200f, 2f)));
		...
		r.each(room -> {
			int roomId = 0;
			// get room to connect
			room.connect(r.get((int) noise3d(((int) sector.tile.v.z/sector.tile.v.y) * 10 + roomId + 20, sector.tile.v, 3, 0.5f, 200f, r.size - 1)));
			...
			erase((int) room.x, (int) room.y, (int) noise3d(((int) sector.tile.v.x/sector.tile.v.z) * 10 + roomId + 30, sector.tile.v, 3, 0.5f, 200f, 12f));
